<div class="card card-info">
	<div class="card-header">
		<h3 class="card-title">
			<i class="fa fa-sync"></i> Hasil Perbandingan Kriteria Pendidikan</h3>
	</div>
	<!-- /.card-header -->
	<div class="card-body">
...
include('fungsi_pendidikan.php');
	...
	for ($x=0; $x <= ($n-2) ; $x++) {
		for ($y=($x+1); $y <= ($n-1) ; $y++) {
			...
			if ($_POST[$pilih] == 1) {
				...
				inputDataPerbandinganPendidikan($x,$y,$matrik[$x][$y]);
	...
	for ($x=0; $x <= ($n-1) ; $x++) {
		...
		// memasukkan nilai priority vektor ke dalam tabel pv_kriteria dan pv_alternatif
			...
			$id_kri = getPendidikanIDI($x);
			inputPendidikanPV($id_kri,$pv[$x]);
...
<?php
	for ($i=0; $i <= ($n-1); $i++) { 
		echo "<th>".getPendidikanNama($i)."</th>";
	}
?>
			</tr>
		</thead>
...
<?php
	for ($x=0; $x <= ($n-1); $x++) { 
		echo "<tr>";
		echo "<td>".getPendidikanNama($x)."</td>";
			for ($y=0; $y <= ($n-1); $y++) { 
...
<?php
	for ($x=0; $x <= ($n-1); $x++) { 
		...
		$id_pendidikan = getPendidikanID($x);
		$vektor = round($pv[$x],2);
		$kali = $vektor * 100;
	    mysqli_query($koneksi,"update pendidikan set bobot='$kali' where id_pendidikan='$id_pendidikan'");
	}
?>
...
<br>
	...
	<a href="index.php?page=data-pendidikan">
